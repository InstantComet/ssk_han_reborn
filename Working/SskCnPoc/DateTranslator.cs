...
internal static class DateTranslator
{
    ...
    public static string? TryTranslateWithTags(string text)
    {
        ...
        lock (CacheLock)
        {
            ...
            if (NoMatch.Contains(text)) return null;
        }
        ...
        {
            lock (CacheLock) { Cache[text] = directResult; }
            return directResult;
        }

        // 尝试处理带标签的情况: <tag>date</tag>
        var tagMatch = Regex.Match(text, @"^(<[^>]+>)(.+?)(</[^>]+>)$");
        if (tagMatch.Success)
        {
            var contentTranslated = TryParseAndTranslate(tagMatch.Groups[2].Value);
            if (contentTranslated != null)
            {
                string result = $"{tagMatch.Groups[1].Value}{contentTranslated}{tagMatch.Groups[3].Value}";
                lock (CacheLock) { Cache[text] = result; }
                return result;
            }
        }
    ...
    private static string? TryTranslateInline(string text)
    {
        ...
        // 替换带标签的日期: <size=75%>March 14th, 1905</size>
        result = InlinePatternWithTag.Replace(result, m =>
        {
            hasMatch = true;
            var translated = FormatChineseDate(m.Groups[4].Value, m.Groups[2].Value, m.Groups[3].Value);
            return translated != null ? $"{m.Groups[1].Value}{translated}{m.Groups[5].Value}" : m.Value;
        });
        ...
        {
            hasMatch = true;
            return FormatChineseDate(m.Groups[3].Value, m.Groups[1].Value, m.Groups[2].Value) ?? m.Value;
        });
